...
build() {
    ...
    cmake --fresh -DTARGET_ARCH=$arch-w64-mingw32 $gcc_arch -DMTUNE=meteorlake -DCOMPILER_TOOLCHAIN=$compiler "${clang_option[@]}" "${pgo_option[@]}" "${arch_option[@]}" $extra_option -DENABLE_CCACHE=ON -DQT_DISABLE_CCACHE=ON -DSINGLE_SOURCE_LOCATION=$srcdir -G Ninja -H$gitdir -B$buildroot/build$bit
    ...
    ninja -C $buildroot/build$bit curl aria2 mediainfo mimalloc
    ...
    llvm-strip -s $buildroot/build$bit/install/$arch-w64-mingw32/bin/aria2c.exe
