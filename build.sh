...
build() {
    ...
    if [ "$arch" == "x86_64" ]; then
        if [ "$x86_64_level" == "-v3" ]; then
            arch_option=(-DMARCH_NAME=-v3 -DMTUNE=meteorlake)
        fi
    fi

    cmake --fresh -DTARGET_ARCH=$arch-w64-mingw32 $gcc_arch -DCOMPILER_TOOLCHAIN=$compiler "${clang_option[@]}" "${pgo_option[@]}" "${arch_option[@]}" $extra_option -DENABLE_CCACHE=ON -DQT_DISABLE_CCACHE=ON -DSINGLE_SOURCE_LOCATION=$srcdir -G Ninja -S$gitdir -B$buildroot/build$bit
